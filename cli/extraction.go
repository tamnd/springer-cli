...
func extractionCmd() *cobra.Command {
	...
	cmd.RunE = func(cmd *cobra.Command, args []string) error {
		...
		if len(fields) == 0 {
			return exit(CodeNoData, fmt.Errorf("no field in the table is answered by the %s rung", rung))
		}
		...
		fmt.Fprintln(tw, "FIELD\tRUNG\tSOURCE\tWHY")
		for _, f := range fields {
			fmt.Fprintf(tw, "%s\t%s\t%s\t%s\n", f.Name, f.Rung, f.Source, f.Why)
		}
		return tw.Flush()
	}
	return cmd
